...
final class Version20190326142921 extends AbstractMigration
{
    ...
    {
        $this->addSql("CREATE TABLE message_history (id UUID NOT NULL, code VARCHAR(50) NOT NULL, recipient_id VARCHAR(255) NOT NULL, sender_id VARCHAR(255) NOT NULL, category VARCHAR(50) DEFAULT 'email' NOT NULL, description VARCHAR(255) DEFAULT NULL, created_at TIMESTAMP(0) WITHOUT TIME ZONE NOT NULL, updated_at TIMESTAMP(0) WITHOUT TIME ZONE NOT NULL, PRIMARY KEY(id))");
        $this->addSql("CREATE INDEX search_by_recipient_idx ON message_history (recipient_id)");
        $this->addSql("CREATE INDEX search_by_sender_idx ON message_history (sender_id)");
